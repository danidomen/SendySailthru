...
<?php 
	...
	if ($r && mysqli_num_rows($r) > 0)
	{
        ...
        $q = 'SELECT email FROM subscribers WHERE id = '.$userID;
	$r = mysqli_query($mysqli, $q);
	...
	if ($r && mysqli_num_rows($r) > 0)
	{
	    while($row = mysqli_fetch_array($r))
	    {
                ...
                $sailthruConfig->onAllListObserver($map_config,$mysqli, $email,false,array('vars'=>array('sendy_last_open' => date('Y-m-d'))));
	    }  
	}
